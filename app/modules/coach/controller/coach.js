  ...
  if (rayon == "0") {
    ...
    await db_library.execute(query_internal).then(async results => {
      if (results.length > 0) {
        ...
        const postal_codes_list = await calculateLocationRadius(
          longitude,
          latitude,
          Code_postal,
          rayon
...
exports.searchByCoachList = async function(req, res, next) {
  ...
  if (rayon == "0") {
    if (course == "CoursIndividuel") {
      query =
        "SELECT `Coach_Id` FROM `individualcourses` WHERE Postalcode = " +
        ville;
    } else if (course == "CoursCollectifOndemand") {
      query =
        "SELECT `Coach_Id` FROM `course_collective_if_demand` WHERE Postalcode = " +
        ville;
    } else if (course == "CoursCollectifClub") {
      query =
        "SELECT `Coach_Id` FROM `couse_collective_if_club` WHERE Postalcode = " +
        ville;
    }
    ...
    await db_library.execute(query).then(async value => {
      if (value.length > 0) {
        ...
        query_data =
          "SELECT DISTINCT (c.Coach_ID), c.Coach_Fname, c.InstagramURL, c.TwitterURL,c.FacebookURL, c.Coach_Phone, c.Coach_Lname, c.Coach_Email, c.Coach_Price, c.Coach_PriceX10, c.Coach_Description, c.Coach_Services, u.Id FROM coaches_dbs c inner join users u on c.Coach_Email = u.email left join avaiablity a on u.id = a.CoachId WHERE u.id IN (" +
          arrData +
        ...
        if (value.length > 0) {
          if (course == "Stage") {
            ...
            for (let i = 0; i < value.length; i++) {
              ...
              const resultCurrentYear = new Date(
                value[i].to_date
              ).getFullYear();
              if (
...
exports.getCoachByPostalcode = async function(req, res, next) {
  ...
  var query =
    "SELECT DISTINCT c.Coach_Fname, c.Coach_ID, c.InstagramURL, c.TwitterURL,c.FacebookURL, c.Coach_Phone, c.Coach_Lname, c.Coach_Email, c.Coach_Price, c.Coach_PriceX10, c.Coach_Description, c.Coach_Services, u.Id FROM coaches_dbs c inner join users u on c.Coach_Email = u.email left join avaiablity a on u.id = a.CoachId WHERE u.roleId = 2 AND u.isActive = 1 AND u.postalCode = '" +
    code +
    "'";
  await db_library
    .execute(query)
...
exports.get_coach_by_id = async function(req, res, next) {
  ...
  if (coach_email != "") {
    await db_library
      ...
      .then(value => {
        if (value.length > 0) {
          //console.log("coach.js-454-", value[0].Coach_ID)
          //setTimeout(() => {
          var obj = {
            coach_list: value
          };
...
async function bookedFun(arr, remaingSlotStatus, totalAmt) {
  try {
    ...
    const Query =
      "Insert into `booking_dbs` (`Coach_ID`,`user_Id`,`payment_Id`,`status`,`bookingDate`,`bookingCourse`,`amount`,`BookingTime`,`Remarks`,`remaingSlotStatus`) values ('" +
      P_CoachId +
      "','" +
      P_UserId +
      "','0','R','" +
...
exports.getReservations = async function(req, res, next) {
  ...
  if (Coach_id != "") {
    var Qry =
      `select booking_Id,BookingTime,Coach_ID,amount,bookingCourse, d.CourseName,(select DATE_FORMAT(bookingDate, '%Y-%m-%d')) as bookingDate,discount_club,paymentStatus,payment_Id,status,user_Id, u.firstName, u.lastName,s.Remarks, s.remaingSlotStatus from booking_dbs s
