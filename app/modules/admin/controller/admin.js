...
exports.registerAdmin = async function(req, res, next) {
    ...
    if (first_name != "" && last_name != "" && admin_email != "" && mobile != "" && password != "" && role != "") {
        var encry_pass = await bcrypt.hash(password, 10);
        

        var query = "INSERT INTO `admin`(`first_name`, `last_name`, `admin_email`, `password`, `mobile`, `address`, `role`)" +
            " VALUES ('" + first_name + "','" + last_name + "','" + admin_email + "','" + encry_pass + "','" + mobile + "','" + address + "','" + role + "')";
        console.log(query);
        await db_library
        
            .execute("SELECT * FROM `admin` WHERE admin_email='" + admin_email + "'").then(async(value) => {
            var result = value;
            ...
            if (result.length == 0) {
                try {
                    ...
                    var temp2 = temp1.replace("{{username}}", first_name + " " + last_name);
...
exports.updateProfile = async function(req, res, next) {
    ...
    const {
        ...
        if (first_name != "" &&  last_name != "" && admin_email != "" && mobile != "" && address != "" &&  role != "") {

    
        var admin_query = "UPDATE `admin` SET `first_name` = '" + first_name + "', `last_name`='" + last_name + "', `admin_email`='" + admin_email + "', `mobile`='" + mobile + "', `address`='" + address + "', `role`='" + role + "' WHERE `admin_id`='" + admin_id + "';";


        await db_library.execute(admin_query).then((value) => {
            _output.data = {};
            _output.isSuccess = true;
...
exports.loginAdmin = async function(req, res, next) {
    ...
    if (admin_email != "" && password != "") {
        const query = "SELECT `admin_id`, `first_name`, `last_name`, `admin_email`, `password`, `mobile`, `address`, " +
            "`role`" +
            " FROM `admin` WHERE `admin_email`= '" + admin_email + "' AND `status`= 1;";
    ...
    if (email != "") {
        ...
        try {
            if (email) {
                ...
                //Query
                var query = "UPDATE `admin` SET `hashKey`='" + encry_hash + "' WHERE `admin_email` = '" + email + "'";
                var sel_query = "SELECT `first_name`,`last_name` from `admin` WHERE `admin_email` = '" + email + "'";
                ...
                await db_library.execute(sel_query).then(async(val) => {
                    if (val.length > 0) {
                        await db_library
                            .execute(query).then(async(value) => {
                                var mailTemplate = await mail_template.getMailTemplate(appConfig.MailTemplate.AdminForgotPassword);
                                const mailOption = require('../../_mailer/mailOptions');
                                let _mailOption = new mailOption();
                                _mailOption.to = email;
                                _mailOption.subject = "Forgotten Password"
                                _mailOption.html = mailTemplate[0].template.replace('{{username}}', val[0].first_name + " " + val[0].last_name).replace('{{hashkey}}', encry_hash);
                                var _mailer = require('../../_mailer/mailer');
                                _mailer.sendMail(_mailOption);
                                _output.data = value;
                                _output.isSuccess = true;
...
exports.resetPassword = async function(req, res, next) {
    ...
    if (email != "" && password != "") {
        var encry_pass = await bcrypt.hash(password, 10);
        if (hashKey == "") {
            var query = "UPDATE `admin` SET `password`= '" + encry_pass + "' WHERE `admin_email` = '" + email + "'";
        } else {
            var query = "UPDATE `admin` SET `password`= '" + encry_pass + "' WHERE `hashKey`='" + hashKey + "' AND `admin_email` = '" + email + "'";
        }
        await db_library
...
exports.changePassword = async function(req, res, next) {
    ...
    if (password != "" && newpassword != ""){
        if(password != newpassword) {
                ...
                var query = "SELECT `password` from `admin` WHERE admin_id = '" + admin_id + "'";
                ...
                await db_library
                    .execute(query).then(async(value) => {
                        ...
                        if(match){
                            ...
                            var updatequery = "UPDATE `admin` SET password = '" + new_encry_pass + "' WHERE admin_id = '" + admin_id + "'; ";

                            await db_library
                                .execute(updatequery).then((value) => {
                                    var result = value;
                                    _output.data = result;
                                    _output.isSuccess = true;
...
exports.getAdminDetails = async function(req, res, next) {
    ...
    await db_library
        .execute("SELECT * FROM `admin`").then((value) => {
            var obj = {
                admin_list: value
            }
            _output.data = obj;
...
exports.getadminbyid = async function (req, res, next) {
    ...
    if (admin_id != "") {
        await db_library
            .execute("SELECT * FROM `admin` WHERE admin_id='" + admin_id + "'").then((value) => {
                if (value.length > 0) {
                    var obj = {
                        admin_list: value
                    }
...
exports.getallcount = async function (req, res, next) {
    ...
    var coachcount;
    await db_library
        .execute("SELECT count(*) FROM `tenniscourt`").then((value) => {
            var obj = {
    ...
    await db_library
        .execute("SELECT count(*) FROM `users` where roleId = 1").then((value) => {
            var obj = {
    ...
    await db_library
        .execute("SELECT count(*) FROM `coaches_dbs`").then((value) => {
...
exports.changeadminstatus = async function (req, res, next) {
    ...
    if (admin_id != "") {
        await db_library
            .execute("UPDATE `admin` SET `status` = IF(status=1, 0, 1) WHERE admin_id='" + admin_id + "'").then((value) => {
